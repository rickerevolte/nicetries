...
app.get("/database", (req, res) => {
  // make a query to get all data from our table
    pool.query("SELECT * FROM mytodo", (error, results, fields) => {
...
app.delete("/database:id", (req, res) => {
  // console.log(`app.delete receiving order to delete`);
  // pool.query(`DELETE FROM mytodo WHERE id=20`, //funktioniert mit fester id
  pool.query(`DELETE FROM mytodo WHERE id=${Id}`,
    req.params.id,  //ist hier etwas falsch??
    (error, results, fields) => {
      // did we have a mysql error?
      if (error) {
        // yes: returning the error
